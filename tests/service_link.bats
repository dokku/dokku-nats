...
teardown() {
  dokku "$PLUGIN_COMMAND_PREFIX:destroy" ms -f
...
@test "($PLUGIN_COMMAND_PREFIX:link) exports NATS_URL to app" {
  ...
  echo "output: $output"
  echo "status: $status"
  url=$(dokku config:get my-app NATS_URL)
  password="$(sudo cat "$PLUGIN_DATA_ROOT/ls/PASSWORD")"
  assert_contains "$url" "nats://ls:$password@dokku-nats-ls:4222"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) generates an alternate config url when NATS_URL already in use" {
  dokku config:set my-app NATS_URL=nats://host:4222
  dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
  run dokku config my-app
  assert_contains "${lines[*]}" "DOKKU_NATS_AQUA_URL"
  assert_success

  dokku "$PLUGIN_COMMAND_PREFIX:link" ms my-app
  run dokku config my-app
  assert_contains "${lines[*]}" "DOKKU_NATS_BLACK_URL"
  assert_success
...
@test "($PLUGIN_COMMAND_PREFIX:link) links to app with docker-options" {
  ...
  assert_contains "${lines[*]}" "--link dokku.nats.ls:dokku-nats-ls"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) uses apps NATS_DATABASE_SCHEME variable" {
  dokku config:set my-app NATS_DATABASE_SCHEME=nats
  dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
  url=$(dokku config:get my-app NATS_URL)
  password="$(sudo cat "$PLUGIN_DATA_ROOT/ls/PASSWORD")"
  assert_contains "$url" "nats://ls:$password@dokku-nats-ls:4222"
