...
@test "($PLUGIN_COMMAND_PREFIX:promote) changes NATS_URL" {
  ...
  dokku config:set my-app "NATS_URL=nats://u:p@host:4222/db" "DOKKU_NATS_BLUE_URL=nats://l:$password@dokku-nats-l:4222"
...
@test "($PLUGIN_COMMAND_PREFIX:promote) uses NATS_DATABASE_SCHEME variable" {
  password="$(sudo cat "$PLUGIN_DATA_ROOT/l/PASSWORD")"
  dokku config:set my-app "NATS_DATABASE_SCHEME=nats2" "NATS_URL=nats://u:p@host:4222/db" "DOKKU_NATS_BLUE_URL=nats2://l:$password@dokku-nats-l:4222"
  dokku "$PLUGIN_COMMAND_PREFIX:promote" l my-app
  url=$(dokku config:get my-app NATS_URL)
